...
```bash
root@benchmark-client-57b7d96cf4-s85rv:~# python3 /tmp/generate_mixed_dataset.py 1000
...
root@benchmark-client-57b7d96cf4-s85rv:~# echo "=== Cold Start TTFT (1 request) ==="
curl -s -w "\nHTTP_CODE:%{http_code}\nTIME_TOTAL:%{time_total}\n" \
  http://vllm-service.llm-inference.svc.cluster.local:8000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"qwen3-06b","messages":[{"role":"user","content":"什么是Kubernetes？"}],"max_tokens":100,"stream":false}' \
...
root@benchmark-client-57b7d96cf4-s85rv:~# echo "=== Warm Cache TTFT (5 sequential identical requests) ==="
for i in {1..5}; do
  START=$(date +%s%N)
  curl -s http://vllm-service.llm-inference.svc.cluster.local:8000/v1/chat/completions \
    -H "Content-Type: application/json" \
...
```bash
root@benchmark-client-57b7d96cf4-s85rv:/tmp# python3 /tmp/extract_metrics.py
