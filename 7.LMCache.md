...
> **架构流程**：`Ingress Controller → vLLM Instance → kv_connector → LMCache Server`
>
> vLLM 通过 `kv_connector` 将 KV Cache 存储到远程 LMCache Server，实现跨 Pod 的缓存共享与命中。

---

## LMCache Server 主要占用的是 CPU 内存（RAM），而不是 GPU 显存。
---
#### LMCache Server 的核心设计目标之一，正是将 KV Cache 从昂贵的 GPU 显存中“卸载”出来，存入更充裕的 CPU 内存中。

- **L0: GPU 显存 (VRAM)**
  - 主要存储介质：GPU 显存
  - 核心特点：速度最快，用于当前正在处理的请求的 KV Cache。这一层通常由推理引擎（如 vLLM、SGLang）自身管理。
- **L1: 高速缓存层**
  - 主要存储介质：CPU 内存 (RAM)
  - 核心特点：LMCache Server 的核心存储池。作为“热缓存”，用于存放近期或频繁访问的 KV Cache，以实现快速复用。
- **L2: 持久化存储层**
  - 主要存储介质：本地磁盘 (SSD/HDD)、远程存储（如 Redis、S3）等
  - 核心特点：大容量、持久化。用于长期保存 KV Cache，即使服务器重启，数据也不会丢失。



> IngressController -> vLLM Instance Deployment -> LMCache Server StatefulSet

> vllm -> kv_connector -> LMCache Server
---

### 1. LMCache Server（StatefulSet + Service）
...
spec:
  ...
  template:
    ...
    spec:
      #nodeSelector:
      #  kubernetes.io/hostname: k8s-node02.demo.com
      containers:
      - name: server
        ...
        args:
          ...
          - "--port"
          - "5555"
          - "--l1-size-gb"         # Prefix Cache 存储空间
          - "10"                   # 10 GB
          - "--eviction-policy"    # 缓存淘汰算法
          - "LRU"                  # LRU（最近最少使用）
          - "--chunk-size"         # 大于此值的缓存块才进行缓存
          - "256"
        ports:
        - containerPort: 5555
          name: zmq
        ...
        resources:
          requests:
            memory: "6Gi"              # 内存需大于 --l1-size-gb 的值
            cpu: "4"
            nvidia.com/gpu: "1"
          limits:
            memory: "16Gi"
...
### 2. vLLM StatefulSet（连接远程 LMCache）

> 通过 `--kv-transfer-config` 和 `LMCACHE_REMOTE_URL` 环境变量，vLLM 将 KV Cache 发送到远程 LMCache Server。
...
# (APIServer pid=1) INFO 07-12 09:43:53 [loggers.py:259] Engine 000:
#   Avg prompt throughput: 0.3 tokens/s, Avg generation throughput: 30.0 tokens/s,
#   Running: 0 reqs, Waiting: 0 reqs, GPU KV cache usage: 0.0%,
#   Prefix cache hit rate: 70.0%, External prefix cache hit rate: 6.6%
```

> 在日志中可以看到 `Prefix cache hit rate: 70.0%`，多跑几次命中率会更高。
>
> **70%** 的请求直接命中缓存，TTFT（Time To First Token）显著降低。
